...
class DownBlockMLX(nn.Module):
    ...
    def transformer_blocks(self):
        return [getattr(self, f'layers_1_{i}') for i in range(self.num_transformer_blocks)]
        ...
        for i in range(len(channels)):
            ...
            if is_last:
                if causal:
                    ...
                    downsample = nn.Conv1d(output_channel, output_channel, kernel_size=3, padding=1)
            else:
                downsample = Downsample1DMLX(output_channel)
            
            # Use explicit attribute name for MLX tracking
            setattr(self, f'down_blocks_{i}', DownBlockMLX(resnet, transformer_blocks, downsample))
        ...
        for i in range(len(reversed_channels) - 1):
            ...
            if is_last:
                if causal:
                    ...
                    upsample = nn.Conv1d(output_channel, output_channel, kernel_size=3, padding=1)
            else:
                upsample = Upsample1DMatchaMLX(output_channel, use_conv_transpose=True)
            
            setattr(self, f'up_blocks_{i}', UpBlockMLX(resnet, transformer_blocks, upsample))
        
        # Final layers
        if causal:
            self.final_block = CausalBlock1DMLX(reversed_channels[-1], reversed_channels[-1])
        else:
    ...
    def __call__(
        ...
        for i in range(self.num_down_blocks):
            ...
            attn_mask = add_optional_chunk_mask(
                x, mask_down.astype(mx.bool_), False, False, 0, self.static_chunk_size, -1
            )
        ...
        for i in range(self.num_mid_blocks):
            ...
            attn_mask = add_optional_chunk_mask(
                x, mask_mid.astype(mx.bool_), False, False, 0, self.static_chunk_size, -1
            )
            attn_mask = mask_to_bias(attn_mask, x.dtype)
