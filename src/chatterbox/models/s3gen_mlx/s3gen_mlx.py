...
class S3Token2MelMLX(nn.Module):
    ...
    def __init__(
        ...
        self.speaker_encoder = CAMPPlusMLX(
            feat_dim=80,
            embedding_size=spk_embed_dim
        )
    ...
    def embed_ref(
        ...
        return dict(
            ...
            prompt_token_len=int(ref_token_lens[0]) if ref_token_lens is not None else (ref_tokens.shape[1] if ref_tokens is not None else 0),
...
def convert_s3gen_weights(pytorch_state_dict: dict) -> dict:
    ...
    key_mappings = {
        # Speaker encoder (CAMPPlus) - uses 'xvector' prefix
        'speaker_encoder.': 'speaker_encoder.',
        
        # Flow module mappings
        'flow.encoder.': 'flow.encoder.',
        'flow.decoder.': 'flow.decoder.',
        'flow.': 'flow.',
        
        # Mel2Wav (HiFiGAN) mappings
        'mel2wav.': 'mel2wav.',
        
        # Tokenizer mappings (S3 tokenizer embedded in S3Gen)
        'tokenizer.': 'tokenizer.',
    }
...
def load_s3gen_mlx(
    ...
    model_class: str = "S3Token2WavMLX"
) -> nn.Module:
    """
