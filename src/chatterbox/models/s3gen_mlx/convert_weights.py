...
def convert_weight_norm(weight_g: np.ndarray, weight_v: np.ndarray) -> np.ndarray:
    ...
    if len(weight_v.shape) == 3:  # Conv1d
        norm = np.linalg.norm(weight_v.reshape(weight_v.shape[0], -1), axis=1, keepdims=True)
...
def map_hifigan_name(key: str) -> str:
    ...
    patterns = [
        ...
        (r'\.source_resblocks\.(\d+)\.convs1\.(\d+)(\.|\Z)', r'.source_resblocks_\1.convs1_\2\3'),
        (r'\.source_resblocks\.(\d+)\.convs2\.(\d+)(\.|\Z)', r'.source_resblocks_\1.convs2_\2\3'),
        (r'\.source_resblocks\.(\d+)\.activations1\.(\d+)(\.|\Z)', r'.source_resblocks_\1.activations1_\2\3'),
        (r'\.source_resblocks\.(\d+)\.activations2\.(\d+)(\.|\Z)', r'.source_resblocks_\1.activations2_\2\3'),
        ...
        (r'\.resblocks\.(\d+)\.activations1\.(\d+)(\.|\Z)', r'.resblocks_\1.activations1_\2\3'),
        (r'\.resblocks\.(\d+)\.activations2\.(\d+)(\.|\Z)', r'.resblocks_\1.activations2_\2\3'),
    ...
    for key, value in pytorch_state_dict.items():
        ...
        if len(np_value.shape) == 3 and 'weight' in key:
            if is_conv_transpose:
                ...
                if verbose:
                    logger.debug(f"ConvTranspose1d transpose: {key} {np_value.shape} -> {new_key} {new_value.shape}")
            else:
                # Conv1d weight
                new_value = convert_conv1d_weight(np_value)
                if verbose:
                    logger.debug(f"Conv1d transpose: {key} {np_value.shape} -> {new_key} {new_value.shape}")
        elif len(np_value.shape) == 4 and 'weight' in key:
            # Conv2d weight
            new_value = convert_conv2d_weight(np_value)
            if verbose:
                logger.debug(f"Conv2d transpose: {key} {np_value.shape} -> {new_key} {new_value.shape}")
        else:
            new_value = np_value
...
def convert_and_load_weights(
    ...
    if exclude_tokenizer:
        ...
        mlx_state = {k: v for k, v in mlx_state.items() if not k.startswith('tokenizer.')}
        filtered_count = original_count - len(mlx_state)
        if filtered_count > 0:
            logger.info(f"Excluded {filtered_count} tokenizer weights (not implemented in MLX)")
    ...
    for key, value in pytorch_state_dict.items():
        ...
        if 'weight_g' in key or 'weight_v' in key:
            ...
            if base_key not in weight_norm_buffers:
                weight_norm_buffers[base_key] = {}
            if 'weight_g' in key:
                weight_norm_buffers[base_key]['weight_g'] = value.numpy() if hasattr(value, 'numpy') else value
            else:
                weight_norm_buffers[base_key]['weight_v'] = value.numpy() if hasattr(value, 'numpy') else value
