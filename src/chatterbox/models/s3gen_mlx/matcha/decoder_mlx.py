...
class CausalConv1DMLX(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, stride=1, padding=0, bias=bias)
...
class DecoderMLX(nn.Module):
    ...
    def __call__(
        ...
        for resnet, downsample in self.down_blocks:
            ...
            x_t = downsample(x_t) if isinstance(downsample, Downsample1DMLX) else downsample(x_t)
        ...
        for resnet, upsample in self.up_blocks:
            ...
            x_t = upsample(x_t) if isinstance(upsample, Upsample1DMatchaMLX) else upsample(x_t)
