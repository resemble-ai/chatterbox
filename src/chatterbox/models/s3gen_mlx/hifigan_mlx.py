...
class ResBlockMLX(nn.Module):
    ...
    def __init__(
        ...
        for i, dilation in enumerate(dilations):
            ...
            conv2 = nn.Conv1d(channels, channels, kernel_size, padding=(kernel_size - 1) // 2)
...
class SineGenMLX(nn.Module):
    ...
    def __call__(self, f0: mx.array) -> tuple:
        ...
        phase_vec = mx.random.uniform(
            ...
            shape=(batch_size, self.harmonic_num + 1, 1)
        )
    ...
    def __init__(
        ...
        for i, (u, k, d) in enumerate(zip(
            ...
            if u == 1:
                ...
                down = nn.Conv1d(in_ch, out_ch, kernel_size=int(u * 2), stride=int(u), padding=int(u // 2))
        ...
        self.conv_post = nn.Conv1d(ch, istft_params["n_fft"] + 2, kernel_size=7, padding=3)
