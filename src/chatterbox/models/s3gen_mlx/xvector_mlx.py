...
def statistics_pooling(x: mx.array, dim: int = -1, keepdim: bool = False, eps: float = 1e-2) -> mx.array:
    """
    ...
    def __init__(self, in_planes: int, planes: int, stride: int = 1):
        ...
        if self.use_shortcut:
            self.shortcut_conv = nn.Conv2d(
                in_planes, self.expansion * planes, kernel_size=1, stride=(stride, 1), bias=False
            )
...
class FCMMLX(nn.Module):
    ...
    def __init__(self, m_channels: int = 32, feat_dim: int = 80, num_blocks: List[int] = [2, 2]):
        super().__init__()
...
class DenseLayerMLX(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, bias: bool = False, affine: bool = False):
        super().__init__()
