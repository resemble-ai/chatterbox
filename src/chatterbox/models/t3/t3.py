...
class T3(nn.Module):
    ...
    def inference_streaming(
        ...
        *,
        ...
        """
        Generator that yields speech tokens one at a time for streaming.
        
        This is a streaming variant of inference() that yields tokens as they
        are generated rather than collecting them all and returning at the end.
        
        Args:
            t3_cond: T3 conditioning (speaker embedding, etc.)
            text_tokens: Tokenized text input (already with SOT/EOT)
        ...
        # Default initial speech to start token
        if initial_speech_tokens is None:
            initial_speech_tokens = self.hp.start_speech_token * torch.ones_like(text_tokens[:, :1])
        ...
        # Set up alignment analyzer for multilingual models
        alignment_stream_analyzer = None
        if self.hp.is_multilingual:
            alignment_stream_analyzer = AlignmentStreamAnalyzer(
                self.tfmr,
        ...
        # Generation loop - yields tokens one at a time
        for i in range(max_new_tokens):
            ...
            cond = logits_step[0:1, :]
            uncond = logits_step[1:2, :]
            cfg = torch.as_tensor(cfg_weight, device=device, dtype=cond.dtype)
            logits = cond + cfg * (cond - uncond)
