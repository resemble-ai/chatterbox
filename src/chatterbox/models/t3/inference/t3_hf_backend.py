...
class T3HuggingfaceBackend(LlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        S should be 1.
        :param max_position: if set, the KVTruncatingStaticCache will truncate key/value states to this
        length before attention, reducing compute for early tokens (bucketed CUDA graph fast path).
        """
        assert return_dict

        # Propagate max_position to the KV cache so KVTruncatingStaticCache.update() can
        # truncate attention to the filled portion of the cache (baked into CUDA graph at capture).
        # Guard against the regular inference path which passes a plain tuple for past_key_values.
