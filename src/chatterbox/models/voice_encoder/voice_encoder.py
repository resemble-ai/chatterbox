...
def trim_audio(wav, top_db=20):
    if isinstance(wav, torch.Tensor):
        wav_np = wav.numpy()
    else:
        wav_np = wav
    
    threshold = 10 ** (-top_db / 20) * np.max(np.abs(wav_np))
    non_silent = np.where(np.abs(wav_np) > threshold)[0]
...
class VoiceEncoder(nn.Module):
    ...
    def embeds_from_wavs(
        ...
        **kwargs
        ...
        if trim_top_db:
            wavs = [trim_audio(wav, top_db=trim_top_db) for wav in wavs]
