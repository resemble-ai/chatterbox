...
# from transformers import pipeline  # Replaced with Gemini API
import torch
...
def compute_loss(
    ...
    for i in range(batch_size):
        text = batch['text'][i]
        language_id = batch['language_id'][i]
        tokens = model.tokenizer.text_to_tokens(text, language_id=language_id).to(device)
...
def main():
    ...
    # Gemini API doesn't need to be moved to CPU
    ...
    # Copy tokenizer
    import shutil
    tokenizer_path = Path(hf_hub_download(repo_id="ResembleAI/chatterbox", filename="grapheme_mtl_merged_expanded_v1.json"))
    shutil.copy(tokenizer_path, merged_dir / "grapheme_mtl_merged_expanded_v1.json")
...
def load_audio_samples(audio_dir: str) -> List[AudioSample]:
    ...
    This function replaces the original Gemini-based transcription approach.
    It reads audio file paths and transcriptions directly from metadata.csv.
    
    Args:
        audio_dir: Base directory containing audio files and metadata.csv
                  Example structure:
                  audio_dir/
                  ├── metadata.csv
                  └── audio/
    ...
    Returns:
        List of AudioSample objects ready for training
    """
        
    samples = []
    
    # Construct path to metadata.csv
    metadata_path = Path(audio_dir) / "metadata.csv"
    
    # Verify metadata file exists
    if not metadata_path.exists():
        raise FileNotFoundError(
            f"metadata.csv not found at {metadata_path}\n"
            f"Please ensure your metadata.csv is in the directory: {audio_dir}\n"
            f"Expected structure:\n"
            f"  {audio_dir}/\n"
            f"  ├── metadata.csv\n"
            f"  └── audio/\n"
            f"      ├── utterance_0001.wav\n"
            f"      └── ..."
        )
    ...
    required_columns = ['file_name', 'transcription']
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        raise ValueError(
            f"Missing required columns in metadata.csv: {missing_columns}\n"
            f"Available columns: {df.columns.tolist()}\n"
            f"Please ensure your metadata.csv has 'file_name' and 'transcription' columns."
        )
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Loading audio samples"):
        try:
            # Get the audio file path (relative to audio_dir)
            # Example: "audio/utterance_0001.wav"
            audio_rel_path = row['file_name']
            audio_path = Path(audio_dir) / audio_rel_path
            
            # Check if file exists
            if not audio_path.exists():
                skipped_not_found += 1
                if skipped_not_found <= 5:  # Only print first 5 warnings
            ...
            # Skip empty or NaN transcripts
            if not transcript or pd.isna(transcript):
                skipped_empty_transcript += 1
                if skipped_empty_transcript <= 5:
                    print(f"Warning: Empty transcript for {audio_path.name}")
                continue
            
            # Get duration from metadata if available (faster)
            # Otherwise load audio to compute it
            if 'duration_seconds' in df.columns and pd.notna(row['duration_seconds']):
                duration = float(row['duration_seconds'])
                # Load just a tiny bit to get sample rate (fast)
                _, sr = librosa.load(audio_path, sr=None, duration=0.1)
            else:
                # Load full audio to get duration (slower)
            ...
            if duration < MIN_AUDIO_LENGTH:
                skipped_too_short += 1
                if skipped_too_short <= 5:
                    print(f"Skipping {audio_path.name}: too short ({duration:.2f}s < {MIN_AUDIO_LENGTH}s)")
                continue
            
            if duration > MAX_AUDIO_LENGTH:
                skipped_too_long += 1
                if skipped_too_long <= 5:
                    print(f"Skipping {audio_path.name}: too long ({duration:.2f}s > {MAX_AUDIO_LENGTH}s)")
                continue
            ...
            skipped_errors += 1
            if skipped_errors <= 5:
                print(f"Error processing row {idx} ({row.get('file_name', 'unknown')}): {e}")
            continue
    ...
    print(f"Skipped (empty transcript): {skipped_empty_transcript}")
    ...
    if samples:
        ...
        transcript_lengths = [len(s.transcript) for s in samples]
        ...
        print(f"    Total: {sum(durations)/60:.2f} minutes")
        print(f"  Transcript length:")
        print(f"    Min: {min(transcript_lengths)} chars")
        print(f"    Max: {max(transcript_lengths)} chars")
        print(f"    Mean: {sum(transcript_lengths)/len(transcript_lengths):.0f} chars")
        print(f"{'='*60}\n")
    else:
        print("\n⚠️  WARNING: No samples loaded! Please check your data.")
